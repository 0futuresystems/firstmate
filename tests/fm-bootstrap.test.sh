...
test_network_phase_partitions_the_run() {
  ...
  # and the network GitHub-auth probe. quota-axi exists only in the fake
  # toolchain, unlike node, which some supported systems install in /usr/bin.
  rm -f "$fakebin/quota-axi"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
exit 1
SH
  ...
  all_out=$(PATH="$fakebin:$(fm_test_base_path_sans "$BASE_PATH" node)" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  assert_contains "$all_out" "MISSING: quota-axi (install:" "the unsplit run lost its local diagnostic"
  ...
  assert_contains "$skip_out" "MISSING: quota-axi (install:" "the local half lost its own diagnostic"
  ...
  assert_not_contains "$only_out" "MISSING: quota-axi" "the network half repeated the local half's work"
