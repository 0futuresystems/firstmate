...
const visibility = await globalThis.fmImportTs(pathToFileURL(`${process.cwd()}/lib/fm-calm-visibility.ts`).href);
...
async function loadCalmExtension() {
  ...
  const extension = await globalThis.fmImportTs(`${pathToFileURL(process.env.EXT).href}?instance=${Date.now()}-${Math.random()}`);
  (extension.default?.default ?? extension.default)(pi);
