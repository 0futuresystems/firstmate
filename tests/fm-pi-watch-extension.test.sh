...
(mod.default?.default ?? mod.default)(previous.pi);
...
(replacementMod.default?.default ?? replacementMod.default)(replacement.pi);
...
(originalMod.default?.default ?? originalMod.default)(original.pi);
...
(finalMod.default?.default ?? finalMod.default)(finalSession.pi);
...
(replacementMod.default?.default ?? replacementMod.default)(pi);
...
test_pi_late_retiring_actionable_reaches_replacement() {
  local repo home plugin foreign_plugin replacement_plugin count out status
  ...
  install_pi_watch_extension_fixture "$repo"
  plugin="$repo/.pi/extensions/fm-primary-pi-watch.ts"
  foreign_plugin="$repo/.pi/extensions/fm-primary-pi-watch-foreign.ts"
  replacement_plugin="$repo/.pi/extensions/fm-primary-pi-watch-replacement.ts"
  cp "$plugin" "$foreign_plugin"
  cp "$plugin" "$replacement_plugin"
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
count=0
...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(PLUGIN="$plugin" FOREIGN_PLUGIN="$foreign_plugin" REPLACEMENT_PLUGIN="$replacement_plugin" FM_HOME="$home" FM_ROOT_OVERRIDE="$repo" FM_ARM_COUNT="$count" FM_WATCH_ARM_RETIRE_TIMEOUT_MS=20 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const foreignMod = await import(pathToFileURL(process.env.FOREIGN_PLUGIN).href);
const foreign = makePi();
(foreignMod.default?.default ?? foreignMod.default)(foreign.pi);
...
const replacementMod = await import(pathToFileURL(process.env.REPLACEMENT_PLUGIN).href);
const replacement = makePi();
(replacementMod.default?.default ?? replacementMod.default)(replacement.pi);
await replacement.handlers.get("session_start")?.({ type: "session_start", reason: "new" }, {});
await waitFor(
...
EOF
  ...
  status=$?
  expect_code 0 "$status" "Pi replacement must receive an actionable close after retirement timeout: $out"
...
for (let moduleIndex = 1; moduleIndex <= 2; moduleIndex += 1) {
  ...
  (mod.default?.default ?? mod.default)(instance.pi);
