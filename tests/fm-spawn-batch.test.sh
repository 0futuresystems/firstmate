...
# An empty config dir, not the checkout's own. A real operating home keeps local
# config/ files here, and config/crew-dispatch.json makes fm-spawn demand an
# explicit resolved harness - a backstop these argument-routing cases never
# supply. Pointing at an empty directory keeps the assertions about argument
# routing rather than about whichever config the host happens to carry.
