...
pi_launcher="$FM_ROOT/bin/fm-pi.sh"
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    line=${line//__FM_PI_LAUNCHER__/$pi_launcher}
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    pi|pi-signed)
      printf '%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi, or restart Pi through ' "$pi_launcher" ' if the extensions are not loaded.'
      ;;
    omp)
