...
Pi's `.pi/extensions/fm-primary-pi-watch.ts` owns initial cycle establishment once its process owns the session lock and continuous re-arm after an actionable child close; OpenCode's `.opencode/plugins/fm-primary-watch-arm.js` owns its continuous re-arm after an actionable child close.
Pi's post-lock `agent_settled` handler waits for initial watcher readiness before the later-loaded turn-end guard runs, so the guard cannot race a lifecycle-owned start.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi lifecycle-owned fresh and replacement generation establishment, post-lock readiness ordering, primary-marker isolation from descendant Pi invocations, repair-only tool metadata, and ownership-based redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers, blocks prompt delivery to prove the successor launches first, verifies single-flight behavior, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure.
...
The goal is lifecycle-owned continuity without a Pi or OpenCode model-memory re-arm step.
