...
Pi's `.pi/extensions/fm-primary-pi-watch.ts` owns initial cycle establishment once its process owns the session lock and continuous re-arm after an actionable child close; omp's `.omp/extensions/fm-primary-omp-watch.ts` and OpenCode's `.opencode/plugins/fm-primary-watch-arm.js` own continuous re-arm after an actionable child close.
...
The goal is lifecycle-owned continuity without a Pi, omp, or OpenCode model-memory re-arm step.
