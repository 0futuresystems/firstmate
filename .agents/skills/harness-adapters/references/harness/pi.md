...
Pi primaries start through `../../../bin/fm-pi.sh`, which disables cwd-dependent discovery, loads the watcher before the turn-end guard by absolute path, binds their loaded markers to the launcher identity, and avoids global installation or project-trust dependence.
The Pi lifecycle waits for the post-lock initial cycle to become ready before the later-loaded guard runs, establishes fresh generations, and re-arms ordinary cycles.
`fm_watch_arm_pi` is repair-only and never a foreground shell arm.
